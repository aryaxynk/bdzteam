  ...
  card.innerHTML=`
    ...
    <div style="display:flex;gap:12px;align-items:stretch;flex-wrap:wrap">
      <div style="flex:1;min-width:260px;position:relative">
        <svg id="chart" viewBox="0 0 640 240" width="100%" height="240" style="display:block"></svg>
      ...
      <div style="width:140px;display:flex;flex-direction:column;gap:10px;justify-content:center">
        <div class="stat" style="margin:0;text-align:center">
          <b>Hôm nay</b>
          <strong id="sTodayKeys" style="font-size:28px;display:block;margin-top:4px">0</strong>
          <span style="font-size:11px;color:var(--muted)">key đã lấy</span>
        </div>
        <div class="stat" style="margin:0;text-align:center">
          <b>Cả tháng</b>
          <strong id="sMonthKeys" style="font-size:22px;display:block;margin-top:4px">0</strong>
          <span style="font-size:11px;color:var(--muted)">key</span>
        </div>
      </div>
    </div>`;
...
function drawMonthChart(series, meta){
  const svg=$('chart'); if(!svg) return;
  const tip=$('chartTip');
  const W=640, H=240, padL=36, padR=16, padT=24, padB=40;
  const data=(series||[]).map(s=>({day:s.day, keys:Number(s.keys||0)}));
  if(!data.length){ svg.innerHTML='<text x="320" y="120" text-anchor="middle" fill="currentColor" font-size="13" opacity=".45">Chưa có dữ liệu</text>'; return; }
  ...
  for(let i=0;i<data.length;i++){
    ...
    if(i% Math.max(1, Math.floor(n/8))===0 || i===n-1){
      const label=String(data[i].day).slice(8);
      html+='<text x="'+cx+'" y="'+(H-14)+'" text-anchor="middle" font-size="10" fill="currentColor" opacity=".55">'+label+'</text>';
    }
  }
  svg.innerHTML=html;
  ...
  function showTip(i){
    ...
    const d=data[i];
    const dd=String(d.day).slice(8)+'/'+String(d.day).slice(5,7);
...
(async()=>{
  for(let i=0;i<40;i++){ if($('pg-dash')&&$('chart')) break; await new Promise(r=>setTimeout(r,100)); }
