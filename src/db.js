...
export const rpc = (env, name, body = {}) => sb(env, "/rpc/" + String(name), { method: "POST", body: JSON.stringify(body) });
...
const PROVIDERS = ["link4m", "trafficvn"];
...
export async function saveSlots(env, value) {
  ...
  for (let position = 1; position <= 2; position += 1) {
    ...
    const token = String(source.token || "").trim();
    data[position] = { position, provider, token, quota: Math.max(1, Math.min(100000, parseInt(source.quota || 5, 10) || 5)), enabled: Boolean(provider && token) };
  }
...
export async function securityLog(env, type, ip, detail = "") {
  return sb(env, "security_events", { method: "POST", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ event_type: type, ip_address: ip, detail: String(detail).slice(0, 1000) }) }).catch(() => {});
}
...
export function isBadUserAgent(request) {
  ...
  if (!ua) return true;
  const blocked = ["curl/", "wget/", "python-requests", "python-urllib", "scrapy", "libwww-perl", "httpclient", "go-http-client"];
