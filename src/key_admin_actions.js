...
export async function handleKeyAdminAction(request, env, session) {
  ...
  try {
    if (action === 'create_key') {
      const key_scope = String(b.key_scope || (raw === 'create_get_key' ? 'GET' : 'ADMIN')).toUpperCase() === 'GET' ? 'GET' : 'ADMIN';
    ...
    const row = (await sb(env, `keys?select=id,status,expires_at& id=eq.${id}&limit=1`.replace('& ', '&')).catch(() => []))?.[0];
