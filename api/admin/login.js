...
export default async function handler(req,res){
  ...
  try{
    ...
    if(u===(env.ADMIN_USERNAME||'admin')&&p===env.ADMIN_PASSWORD){const s=await signSession(env,{u,r:'main',exp:Date.now()+8*3600000});const r=json({ok:true,role:'main'},200,{'set-cookie':cookieHeader('bdz_admin',s)});res.statusCode=r.status;r.headers.forEach((v,k)=>res.setHeader(k,v));return r.arrayBuffer().then(x=>res.end(Buffer.from(x)))}
    const rows=await sb(env,'sub_admins?select=id,username,password_hash,status&username=eq.'+encodeURIComponent(u)+'&limit=1').catch(()=>[]),sub=rows?.[0];if(!sub||sub.status!=='ACTIVE'){const r=jsonError('Tài khoản hoặc mật khẩu không chính xác.',401);res.statusCode=r.status;r.headers.forEach((v,k)=>res.setHeader(k,v));return r.arrayBuffer().then(x=>res.end(Buffer.from(x)))}
    const ok=await bcrypt.compare(p,sub.password_hash).catch(()=>false);if(!ok){const r=jsonError('Tài khoản hoặc mật khẩu không chính xác.',401);res.statusCode=r.status;r.headers.forEach((v,k)=>res.setHeader(k,v));return r.arrayBuffer().then(x=>res.end(Buffer.from(x)))}
    const pending=await signSession(env,{sub_id:sub.id,user:sub.username,exp:Date.now()+300000});const r=json({ok:true,requires_auth_code:true},200,{'set-cookie':cookieHeader('bdz_pending',pending,300)});res.statusCode=r.status;r.headers.forEach((v,k)=>res.setHeader(k,v));return r.arrayBuffer().then(x=>res.end(Buffer.from(x)))}
  catch(error){console.error('[admin/login]',error);const r=jsonError('Lỗi máy chủ. Vui lòng thử lại sau.',500);res.statusCode=r.status;r.headers.forEach((v,k)=>res.setHeader(k,v));return r.arrayBuffer().then(x=>res.end(Buffer.from(x)))}
}
