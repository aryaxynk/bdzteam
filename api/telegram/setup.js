...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    await telegram('setWebhook', { url: webhookUrl, secret_token: webhookSecret(), allowed_updates: ['message', 'callback_query'], drop_pending_updates: false });
    await telegram('setMyCommands', { commands: [
      { command: 'start', description: 'Mở menu BDZ Key Bot' },
      { command: 'getkey', description: 'Lấy link GET KEY' }
    ] });
    const info = await telegram('getWebhookInfo', {});
    return json(res, 200, { ok: true, message: 'Telegram webhook configured', webhook: info.result?.url || webhookUrl, pending_update_count: info.result?.pending_update_count || 0, last_error_message: info.result?.last_error_message || null, last_error_date: info.result?.last_error_date || null });
  } catch (error) {
    console.error(error);
