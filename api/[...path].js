import {json,clearAuth,isBadUserAgent,ipOf,banned,autoBanned,getSession} from '../src/db.js';
import {products,startGetKey,claimKey,keyMeta,siteData,publicStats} from '../src/public_api.js';
import {siteGate} from '../src/gate_api.js';
...
function toRequest(req, url) {
  ...
  if (!['GET','HEAD'].includes(req.method)) {
    if (req.body !== undefined && typeof req.body === 'string') body = req.body;
...
export default async function handler(req,res){
  ...
  const path='/'+(Array.isArray(requested)?requested.join('/'):String(requested||'')).replace(/^\/+/, '');
  const url=base+path+(req.url?.includes('?')?'?'+req.url.split('?').slice(1).join('?'):'');
  const request=toRequest(req,url);
  const env=process.env;
  try{
    if(req.method==='OPTIONS') return send(res,new Response(null,{status:204,headers:{'access-control-allow-origin':req.headers.origin||'*','access-control-allow-methods':'GET,POST,OPTIONS','access-control-allow-headers':'content-type','access-control-allow-credentials':'true'}}));
    if(path==='/health') return send(res,json({ok:true,service:'bdzteam-vercel',target:base,turnstile:!!env.TURNSTILE_SECRET_KEY}));
    if(path==='/session-check'&&req.method==='GET')return send(res,await sessionCheck(request,env));
    if(path==='/api/site-gate'&&req.method==='POST')return send(res,await siteGate(request,env));
    ...
    if((path==='/key'||path==='/key.html'||path==='/check-key'||path==='/check-key.html')&&!await (async()=>{const c=request.headers.get('cookie')||'';return !!c.includes('bdz_gate=')})()){
      const {validGateCookie}=await import('../src/gate_api.js');
      if(!(await validGateCookie(request,env))) return res.redirect(302,'/');
    }
    return res.status(404).json({error:'Not found'});
  }catch(e){console.error('[vercel]',e);return send(res,json({error:'Lỗi máy chủ. Vui lòng thử lại sau.'},500));}
}
