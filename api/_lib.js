...
async function createKey(scope = 'quick', telegramUser = null, durationHours = null) {
  ...
  const fallback = cleanScope === 'dev_ys' ? 24 : 24;
  ...
  const keyCode = randomKey(cleanScope);
  const now = new Date();
  const payload = {
    key_code: keyCode,
    key_scope: cleanScope,
    status: 'ACTIVE',
  ...
  if (telegramUser?.id != null) {
    await supabaseFetch('telegram_users', {
      ...
      body: JSON.stringify({ telegram_user_id: telegramUser.id, username: telegramUser.username || null, first_name: telegramUser.first_name || null, last_name: telegramUser.last_name || null, last_seen_at: now.toISOString() })
    });
  }
...
async function recordCheck(item, result, deviceHash, req, appVersion) {
  await supabaseFetch('key_checks', {
    method: 'POST',
    body: JSON.stringify({
      key_id: item.id,
      key_scope: item.key_scope,
      device_id_hash: deviceHash,
      ip_address: firstIp(req),
      result,
      app_version: String(appVersion || '').slice(0, 80) || null
    })
  });
}

async function shortenVuotLink(longUrl) {
  const token = env('VUOTLINK_API_TOKEN');
  const endpoint = `https://vuotlink.xyz/api?api=${encodeURIComponent(token)}&url=${encodeURIComponent(longUrl)}`;
